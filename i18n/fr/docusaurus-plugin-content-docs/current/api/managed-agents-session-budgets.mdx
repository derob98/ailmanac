...
Il y a un départage important : **une demande en attente surclasse le plafond**. Si un thread attend sur `requires_action` (un `user.tool_confirmation`, disons) et un autre est en pause à `budget_reached`, la session rapporte `requires_action` au niveau supérieur — car répondre à cette demande est un [événement de règlement](#quels-événements-la-session-accepte-encore) que le budget ne bloque pas. Votre UI opérateur doit afficher l'invite requires-action en premier.
