...
The related pages on AILmanac cover the neighboring surfaces of this same problem: [MCP Tool Poisoning, Rug Pulls & Agentjacking](/docs/security/mcp-tool-poisoning-and-rug-pulls) for server-side variants, [Coding Agents Under Attack](/docs/security/coding-agents-under-attack) for the coding-specific angle, [Agentic Browsers and Same-Origin Risk](/docs/security/agentic-browsers-same-origin) for the browser-side version, [Cryptographic Context Injection](/docs/security/cryptographic-context-injection-grok) for the trust-laundering-via-code-sandbox cousin, and [Hardening Autonomous Runs](/docs/security/hardening-autonomous-runs) for the hooks-and-trace pattern this page's checklist relies on.
...
- [GhostSplice: The Cross-Channel MCP Attack That Doubles Compliance](/docs/security/ghostsplice-cross-channel-mcp-attack)
