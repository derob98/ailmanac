...
The concept to hold onto: **the browser is becoming a shared surface between humans and agents, not two separate browsers pointed at the same pages.** That reshapes what "computer use" costs and what it can safely be pointed at. See also [Computer-use agents](/docs/models/computer-use-agents) for how Claude's own browser-control feature compares to this pattern, and [Cloudflare Kitesurf](/docs/models/cloudflare-kitesurf-agent-browser) for the *stateless* end of the same spectrum — a browser runtime built for automation rather than shared-login, where the trade is 3-7x less CPU/memory at ~1.7x more wall-clock time.
...
- [Cloudflare Kitesurf: The First Browser Runtime Built for AI Agents (Not Humans)](/docs/models/cloudflare-kitesurf-agent-browser)
