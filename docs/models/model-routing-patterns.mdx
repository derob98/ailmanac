...
- **RAG QA over a document corpus.** Classifier picks between "answer from context" (cheap) and "needs cross-document reasoning" (mid). An ensemble of two models cross-checks the answer for high-stakes documents (contracts, filings). Compare with [Retrieval-Augmented Generation](/docs/foundations/rag).
...
- A purpose-built classifier stage: [System One decision models: Jev, Kev and the open clones](/docs/models/system-one-decision-models-jev-kev) — calibrated probabilities in under half a second, and how to threshold them honestly.
