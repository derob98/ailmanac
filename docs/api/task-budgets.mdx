...
Task budgets are **not** supported on [Claude Code](/docs/claude-code/overview) or Cowork surfaces — use them directly through the Messages API on one of the supported models. If you need a hard dollar cap on a Managed Agents session instead, that's a separate feature: [Managed Agents Session Budgets](/docs/api/managed-agents-session-budgets).
