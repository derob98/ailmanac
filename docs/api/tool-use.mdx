...
<VerifyNote lastVerified="2026-09-21" source="https://platform.claude.com/docs/en/agents-and-tools/tool-use/overview">
...
- **Anthropic-provided tools.** Anthropic also offers **server tools** that run on its infrastructure with no handler code on your side (web search, web fetch, code execution, tool search), plus **Anthropic-schema client tools** whose schema is predefined but which *your* app still executes (bash, text editor, memory, computer use) — check the docs for the current menu.
