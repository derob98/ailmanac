...
- **[Memory stores](/docs/api/managed-agents-memory-stores)** — persistent state across sessions, mounted as a directory in the sandbox, with immutable versioning and a redact endpoint. No database on your side.
...
- [Managed Agents Session Budgets](/docs/api/managed-agents-session-budgets) — the August 2026 beta for hard USD caps on a session (with the budget_reached stop reason)
- [Managed Agents Domain Restrictions](/docs/api/managed-agents-domain-restrictions) — the August 26, 2026 beta for per-tool `allowed_domains` and `blocked_domains` on `web_search` and `web_fetch`
- [Managed Agents Memory Stores](/docs/api/managed-agents-memory-stores) — the July 2026 beta for persistent, versioned agent memory
