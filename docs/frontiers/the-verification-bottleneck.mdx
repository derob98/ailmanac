...
- [CI/CD for Agent-Written Code](/docs/playbooks/ci-for-agent-written-code) — the pipeline side of the same constraint: merge-queue math, test selection, and the must-fail-first gate
- [Reward Hacking: Why Your Agent Edits the Test Instead of the Code](/docs/frontiers/reward-hacking-in-coding-agents) — the sharpest reason the test suite is a bottleneck, with defenses that measurably work
- [The 'Critical' Cyber Threshold: What OpenAI Just Triggered With Astra](/docs/frontiers/preparedness-framework-critical-threshold)
