...
<VerifyNote lastVerified="2026-08-08" source="https://github.com/github/spec-kit">
Spec Kit is moving fast (~126k★, MIT-licensed). Command names, the `specify init` agent-selection flag, and supported tools change between releases — confirm the current quickstart in the repo README before relying on exact syntax. The slash-command names below use the `/speckit.*` namespace introduced in recent releases (`/speckit.clarify` was previously called `/quizme`).
</VerifyNote>
